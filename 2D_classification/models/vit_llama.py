...
class ViTLLaMA(nn.Module):
    ...
    def __init__(
            ...
            llam_res=0, requires_grad=False):
        """
        Args:
    ...
    def forward_features(self, x):
        ...
        if self.llam_res == 1: # residual within nn
            x = self.llama_dim_mapper1(x)
            x = self.llama(x) + x
            x = self.llama_dim_mapper2(x)
        elif self.llam_res == 2: # residual out of nn
            tmp = x
            x = self.llama_dim_mapper1(x)
            x = self.llama(x)
            x = self.llama_dim_mapper2(x)
            x += tmp
        elif self.llam_res == 3: # residual in and out of nn
            tmp = x
            x = self.llama_dim_mapper1(x)
            x = self.llama(x) + x
            x = self.llama_dim_mapper2(x)
            x += tmp
        elif self.llam_res == 4: # no real llm
            tmp = x
            x = self.llama_dim_mapper1(x)
            x = self.llama_dim_mapper2(x)
            x += tmp
...
@register_model
def vit_res_in_llama_small_patch16_224(pretrained=False, **kwargs):
    llama_default_config = {"dim": 4096, "multiple_of": 256,
        "n_heads": 32, "n_layers": 32, "norm_eps": 1.0e-6,
        "vocab_size": -1, "first_layer": 31}
    model = ViTLLaMA(
        patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), llama_configs=llama_default_config, llam_res=1, **kwargs)
    model.default_cfg = _cfg()
    return model


@register_model
def vit_res_out_llama_small_patch16_224(pretrained=False, **kwargs):
    llama_default_config = {"dim": 4096, "multiple_of": 256,
        "n_heads": 32, "n_layers": 32, "norm_eps": 1.0e-6,
        "vocab_size": -1, "first_layer": 31}
    model = ViTLLaMA(
        patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), llama_configs=llama_default_config, llam_res=2, **kwargs)
    model.default_cfg = _cfg()
    return model


@register_model
def vit_res_hybrid_llama_small_patch16_224(pretrained=False, **kwargs):
    llama_default_config = {"dim": 4096, "multiple_of": 256,
        "n_heads": 32, "n_layers": 32, "norm_eps": 1.0e-6,
        "vocab_size": -1, "first_layer": 31}
    model = ViTLLaMA(
        patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), llama_configs=llama_default_config, llam_res=3, **kwargs)
    model.default_cfg = _cfg()
    return model

@register_model
def vit_mlp_small_patch16_224(pretrained=False, **kwargs):
    llama_default_config = {"dim": 4096, "multiple_of": 256,
        "n_heads": 32, "n_layers": 32, "norm_eps": 1.0e-6,
        "vocab_size": -1, "first_layer": 31}
    model = ViTLLaMA(
        patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), llama_configs=llama_default_config, llam_res=4, **kwargs)
    model.default_cfg = _cfg()
    return model
