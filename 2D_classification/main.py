...
def main(args):
    ...
    if args.eval:
        ...
        if not (args.inc_path or args.ina_path or args.inr_path or args.insk_path or args.pgd_test or args.fgsm_test):
            val_stats = evaluate(data_loader_val, model, device, prefix="val")
            test_stats = evaluate(data_loader_test, model, device, prefix="test")
            print(f"Val: accuracy of the network on the {len(dataset_val)} test images: {val_stats['acc1']:.1f}%")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch+1) % args.test_every == 0:
            ...
            if test_stats["acc1"] > test_max_accuracy:
                checkpoint_paths = [os.path.join(output_dir, 'best_model_acc.pth')]
                for checkpoint_path in checkpoint_paths:
                    utils.save_on_master({
                        'model': model_without_ddp.state_dict(),
                        'optimizer': optimizer.state_dict(),
            ...
            if test_stats["auc"] > test_max_auc:
                checkpoint_paths = [os.path.join(output_dir, 'best_model_auc.pth')]
                for checkpoint_path in checkpoint_paths:
                    utils.save_on_master({
                        'model': model_without_ddp.state_dict(),
                        'optimizer': optimizer.state_dict(),
            ...
            val_max_accuracy = max(val_max_accuracy, val_stats["acc1"])
            val_max_auc = max(val_max_auc,val_stats["auc"])
            test_max_accuracy = max(test_max_accuracy, test_stats["acc1"])
            test_max_auc = max(test_max_auc,test_stats["auc"])
            print(f'* Val: max ACC: {val_max_accuracy:.3f}%, max AUC: {val_max_auc:.3f}%')
            print(f'* Test: max ACC: {test_max_accuracy:.3f}%, max AUC: {test_max_auc:.3f}%')
            ...
            if early_stopping(val_loss, args.patience_epochs):
                print("early stop!")
                break
